...
class TestReachabilityAnalyzer:
    ...
    def test_multi_hop_reachability(self):
        ...
        assert any(
            "agent" in p.node_ids
            for p in paths
            if p.source_id == "source" and p.sink_id == "sink"
        )
    ...
    def test_confidence_cross_agent(self):
        ...
        agent1 = NodeDefinition(
            id="agent1",
            name="Agent1",
            type=NodeType.AGENT
        )

        agent2 = NodeDefinition(
            id="agent2",
            name="Agent2",
            type=NodeType.AGENT
        )
        ...
        graph = GraphDefinition(
            framework="Test",
            nodes=[source, agent1, agent2, sink],
            edges=edges
        )
        ...
        if len(paths) > 0:
            ...
            cross_agent_paths = [
                p for p in paths
                if "agent1" in p.node_ids and "agent2" in p.node_ids
            ]
            if cross_agent_paths:
                assert any(
                    p.confidence in [Confidence.MEDIUM, Confidence.LOW]
                    for p in cross_agent_paths
                )
    ...
    def test_identifies_all_sink_types(self):
        ...
        edges = [
            EdgeDefinition(source="source", target=sink.id)
            for sink in sinks
        ]
